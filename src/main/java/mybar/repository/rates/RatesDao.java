...
public interface RatesDao extends JpaRepository<Rate, CocktailToUserPk> {
    ...
    @Query("SELECT new mybar.domain.History(c.name, r.stars, r.pk.user.username) FROM Cocktail c, Rate r WHERE r.pk.cocktail.id = c.id AND r.ratedAt >= :startDate AND r.ratedAt <= :endDate")
